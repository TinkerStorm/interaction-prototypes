...
import { ComponentContext, Permissions, ComponentType, ModalOptions, TextInputStyle, ButtonStyle, MessageOptions } from "slash-create";
  ...
  ctx.sendModal(createModalOptions(), async modalCtx => {
    ...
    const channel = await client.createChannel(ctx.guildID, game.title, 0, {
      ...
      permissionOverwrites: [
        ...categoryChannel.permissionOverwrites.map(({ id, type }) => ({ id, type, allow: observerPermissions.bitfield, deny: 0n })),
